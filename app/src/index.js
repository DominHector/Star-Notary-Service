...
const App = {
  ...
  lookUp: async function (){
    ...
    if (starName.length !== 0){
      ...
      let out_message = '<p>Star owned</p>';  
      out_message += `<p>Star ID: ${id}</p>`;
      out_message += `<p>Star name: ${starName}</p>`;
      out_message += `<p>Token name: ${contract} </p>`;
      out_message += `<p>Token symbol: ${sym} </p>`;

      App.setStatus(out_message,"status");
    }else{
